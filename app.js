...
function openGiphyModal() {
  ...
  body.innerHTML = `
    <p style="color:var(--text-muted);font-size:13px;margin-bottom:10px;">Uses server route <code>/api/giphy/search</code> — set <code>GIPHY_API_KEY</code> in the server environment.</p>
...
async function loadGiphy(q) {
  ...
  try {
    ...
    if (!r.ok) {
      grid.innerHTML = `<span style="color:var(--red)">${escHtml(data.error || "Error")}</span>`;
      return;
    }
...
// --- Members + DMs ---
function renderMembersAndDms(onlineList) {
